...
export const EXERCISE_META = {
  ...
  // [2026-08-06 신규] 일립티컬과 유사하되 궤적(저항 곡선)이 달라 별도 기구로 취급되는
  // 아크트레이너(Arc Trainer). 이미지 데이터셋(ExerciseGymGifsDB)에 대응 GIF가 없어
  // 매핑하지 않고 "이미지 준비중"으로 표시된다(exerciseImageMap.js 참고).
  "아크트레이너": {
    part: "유산소",
    equipment: "machine",
    subRegion: "전신",
...
// [2026-08-06 (2) 수정] workoutLogs 등 저장된 기록의 종목명이 과거 라이브러리 개편/그립
// 통합 이전 이름인 경우, 정규화 없이 바로 조회하면 못 찾고 "기타"로 분류되는 문제가 있었다
// (LEGACY_EXERCISE_NAME_MAP이 존재해도 routineTemplates 로딩 시에만 적용되고 있었음 — 근본
// 원인). 조회 전 normalizeExerciseName()을 먼저 거치도록 해, 이 함수를 쓰는 모든 화면
// (ReportTab 부위별 추이·과부하·Top5, CalendarView, 색상 조회 등)에 일괄 적용되게 했다.
export function getExerciseAtom(name) {
  const normalized = normalizeExerciseName(name)
  for (const [atom, list] of Object.entries(EXERCISE_LIBRARY)) {
    if (list.includes(normalized)) return atom
  }
  return null
}
...
// [2026-08-06 신규] 리포트 탭 레이더차트에 "가슴/등 운동 시 이두·삼두·어깨도 보조적으로
// 개입" 반영(사용자 확인)을 위해, muscleRoles.synergist에 적힌 근육명(예: "삼두근-장두")을
// 8개 원자 부위(가슴/등/어깨/이두/삼두/하체/코어/유산소) 중 하나로 매핑한다. 서브리전
// 표기(-장두/-상부 등)가 붙어 있어도 매칭되도록 부분 문자열(includes)로 비교한다.
// 원 종목의 주동근 부위(primary atom)와 겹치는 항목은 호출부에서 걸러내 중복 가산을
// 막는다(예: 이두 운동의 synergist "상완근"은 이미 이두 atom과 겹치므로 굳이 또 셀 필요
// 없음 — 실제로는 상완근이 이두 atom에 안 걸려 자연히 제외되지만, 방어적으로 처리).
const SYNERGIST_MUSCLE_TO_ATOM = [
  ['이두근', '이두'],
  ['상완근', '이두'], // 상완근(brachialis)은 별도 근육이지만 이 라이브러리에서는 이두 atom으로 편입
  ['상완요골근', '이두'],
  ['삼두근', '삼두'],
  ['삼각근', '어깨'], // 전면/측면/후면삼각근 모두 어깨로 편입
  ['대흉근', '가슴'],
  ['광배근', '등'],
  ['승모근', '등'],
  ['능형근', '등'],
...
export function getSynergistAtoms(name) {
  const synergists = getMuscleRoles(name)?.synergist
  if (!synergists || synergists.length === 0) return []
  const atoms = new Set()
  synergists.forEach((muscle) => {
    const match = SYNERGIST_MUSCLE_TO_ATOM.find(([keyword]) => muscle.includes(keyword))
    if (match) atoms.add(match[1])
  })
  return [...atoms]
}
...
// [2026-08-06 신규] 종목당 별칭 1개(alias, 상세화면 이름 옆 표시용)만으로는 "사레레" 같은
// 국내 줄임말을 폭넓게 지원하기 어려워, 검색 전용 다건 별칭 배열(aliases)을 추가했다.
// 화면 표시(getExerciseAlias)는 기존처럼 alias 1개만 쓰고, 검색(HowToTab.jsx)은 이 함수로
// alias + aliases를 합쳐 매칭한다.
export function getExerciseAliases(name) {
  const meta = EXERCISE_META[name]
  if (!meta) return []
  const list = []
  if (meta.alias) list.push(meta.alias)
  if (meta.aliases) list.push(...meta.aliases)
  return list
}
...
export const EXERCISE_DESCRIPTIONS = {
  ...
  "웨이티드(중량) 딥스": { summary: "체스트딥스에 벨트나 조끼로 중량을 추가한 고급 버전.", howTo: "체스트딥스와 동일한 자세로, 허리에 중량 벨트를 착용하고 진행한다.", tip: "맨몸 딥스를 15회 이상 여유롭게 할 수 있을 때 시도하는 것을 권장합니다." },
  ...
  "웨이티드(중량) 풀업": { summary: "벨트로 중량을 추가한 고급 풀업.", howTo: "풀업과 동일한 자세로, 허리 벨트에 원판을 매달고 진행한다.", tip: "맨몸 풀업을 8회 이상 여유롭게 할 수 있을 때 시도를 권장합니다." },
  ...
  "하이퍼익스텐션(웨이티드-중량)": { summary: "백익스텐션에 원판을 더해 강도를 높인 버전.", howTo: "백익스텐션과 동일한 자세로, 가슴 앞에 원판을 들고 진행한다.", tip: "맨몸 백익스텐션이 쉽게 느껴질 때 단계적으로 중량을 추가하세요." },
  ...
  "핵스쿼트머신(브이스쿼트머신)": { summary: "등을 기댄 채 발판을 밀어내는, 대퇴사두근 집중 머신 운동.", howTo: "어깨와 등을 패드에 기댄 채 발판에 발을 대고, 무릎을 굽혀 내려갔다가 밀어 올린다.", tip: "발 위치를 발판 위쪽에 두면 대퇴사두근에 더 집중됩니다." },
  ...
  "힙어브덕션머신(아웃타이)": { summary: "앉아서 다리를 벌려 중둔근을 자극하는 머신 운동. 헬스장에서 '아웃타이(아웃싸이)'로도 불립니다.", howTo: "머신에 앉아 패드에 다리 바깥쪽을 대고, 다리를 벌렸다가 모은다.", tip: "골반 안정성에 도움이 되는 보조 운동입니다." },
  "힙어덕션머신(이너타이)": { summary: "앉아서 다리를 모아 내전근군을 자극하는 머신 운동. 헬스장에서 '이너타이(이너싸이)'로도 불립니다.", howTo: "머신에 앉아 패드에 다리 안쪽을 대고, 다리를 모았다가 벌린다.", tip: "힙어브덕션과 짝을 이뤄 골반 주변 균형을 잡는 데 좋습니다." },
  ...
  "아크트레이너": { summary: "일립티컬과 비슷하지만 궤적(저항 곡선)이 달라 무릎 부담이 더 적은 유산소 기구.", howTo: "손잡이와 발판을 함께 움직이며 페달링하듯 진행한다.", tip: "일립티컬보다 자연스러운 관절 궤적을 원한다면 시도해보세요." },
...
export const LEGACY_EXERCISE_NAME_MAP = {
  ...
  // [2026-08-06 신규] 종목명 표기 통일(웨이티드 표기 규칙 확정) + 병기 라벨 추가에 따른
  // 옛 이름 → 새 이름 매핑. '덤벨플로어프레스(삼두)'는 원래 가리키던 '덤벨플로어프레스'
  // 자체가 이번 세션에서 완전 삭제되어(사용자 확인: 과거 기록 이름은 매핑 없이 그대로 둠)
  // 더 이상 유효한 대상이 없으므로 매핑에서 제거했다.
  '웨이티드딥스': '웨이티드(중량) 딥스',
  '웨이티드풀업': '웨이티드(중량) 풀업',
  '하이퍼익스텐션(웨이티드)': '하이퍼익스텐션(웨이티드-중량)',
  '핵스쿼트머신': '핵스쿼트머신(브이스쿼트머신)',
  '힙어브덕션머신': '힙어브덕션머신(아웃타이)',
  '힙어덕션머신': '힙어덕션머신(이너타이)',
  // 행잉니레이즈는 이미지 매핑이 끝내 없어 종목 자체를 삭제하고 행잉레그레이즈로 대체
  // (사용자 확인, 2026-08-06). 과거 기록은 행잉레그레이즈로 취급된다.
  '행잉니레이즈': '행잉레그레이즈',

  // [2026-08-06 (2) 신규] CHANGELOG.md "[2026-08-01] 운동 종목명 표기 정리(22건)" 이력을
  // 감사(audit)해 추가. 그 세션에서는 "과거 기록과의 자동 매칭은 신경쓰지 않기로 함"이라고
  // 명시적으로 보류했던 항목들이라, 그립 통합(2026-08-05) 이후 새로 생긴 이름들과 달리 지금껏
  // 매핑에 전혀 없었다 — 이번 "기타" 오분류 문의로 뒤늦게 보강한다. 최종적으로는 항상 "현재
  // 라이브러리에 실제 존재하는 이름"을 가리키도록(정규화는 1단계만 적용되므로 체인 금지).
  '랫풀다운(내로우그립)': '랫풀다운',
  '레그컬(라잉)': '라잉레그컬',
  '레그컬 라잉': '라잉레그컬',
  '레그컬(시티드)': '시티드레그컬',
  '레그컬 시티드': '시티드레그컬',
  '케이블푸시다운(스트레이트바)': '케이블푸시다운',
  '케이블푸시다운(로프)': '케이블푸시다운',
  '스미스플랫프레스': '플랫스미스프레스',
  '머신숄더프레스': '숄더프레스머신',
  '스미스머신스쿼트': '스미스스쿼트',
  '로우케이블플라이': '케이블로우플라이',
  '사이드레터럴레이즈': '덤벨사이드레터럴레이즈',
  '프론트레이즈': '덤벨프론트레이즈',
  '해머컬': '덤벨해머컬',
  '프리처컬': '바벨프리처컬',
  '컨센트레이션컬': '덤벨컨센트레이션컬',
  '업라이트로우': '바벨업라이트로우',
  '페이스풀': '케이블페이스풀',
  '라잉트라이셉스익스텐션': '바벨라잉트라이셉스익스텐션',
  '런지': '덤벨런지',
  '불가리안스플릿스쿼트': '덤벨불가리안스플릿스쿼트',
  '힙쓰러스트': '바벨힙쓰러스트',
  '스탠딩카프레이즈': '바벨스탠딩카프레이즈',
  '시티드카프레이즈': '바벨시티드카프레이즈',
}
