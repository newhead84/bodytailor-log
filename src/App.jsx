 ...
 * 최신 변경: [2026-08-06 (4)] 운동 종목 DB 정리 12건 ① 팬디컬크로스오버/21컬/덤벨플로어
 *            프레스/밴드푸시다운/행잉니레이즈 삭제(레거시 매핑 없이 완전 삭제, 행잉니레이즈만
 *            행잉레그레이즈로 매핑) ② 데드리프트/스모데드리프트 하체에도 중복 등재(루마니안
 *            데드리프트와 동일 패턴) ③ 웨이티드 표기 통일("웨이티드(중량) 딥스/풀업",
 *            "하이퍼익스텐션(웨이티드-중량)") ④ 플레이트레터럴로우 이미지를 더 정확한 GIF
 *            (barbell-rear-delt-row)로 교체 ⑤ 힙어브덕션/힙어덕션에 "(아웃타이)"/"(이너타이)"
 *            병기(서로 다른 운동임을 확인 — 통합하지 않음) ⑥ 핵스쿼트머신에 "(브이스쿼트머신)"
 *            병기 ⑦ 아크트레이너를 일립티컬과 별개 종목으로 신규 추가(이미지 매핑 없음)
 *            ⑧ 리포트탭 레이더차트에 보조근(synergist) 개입 30% 가중치 반영 신규
 *            (`getSynergistAtoms`, `ReportTab.jsx`) ⑨ HOWTO 검색에 다건 별칭(예: 사레레)
 *            지원 추가(`aliases` 배열, 28개 종목). (`exerciseLibrary.js`, `exerciseImageMap.js`,
 *            `ReportTab.jsx`, `HowToTab.jsx`)
