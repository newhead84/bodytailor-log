...
[2026-08-07] 종목 펼치면 인풋 안 뜨는 버그 근본수정(⑯)
- **증상**: NOTE탭에서 종목을 "시작"으로 펼쳤을 때 세트 입력 행(SetRow)이 하나도 안 뜨고
  "세트완료" 버튼만 보이는 상태가 접었다 다시 펼쳐도 계속 유지되는 영구적 버그.
- **근본원인**: `openExercise()`의 세트 초기화 가드가 `records[종목명]`의 "존재 여부"만
  확인해, 빈 배열(`[]`)이 한 번 들어가면 "이미 초기화됨"으로 오판해 이후 몇 번을 다시 펼쳐도
  기본 세트를 채우지 않았다. 초기화 로직에 예기치 못한 에러가 나도 그 뒤 코드가 아예
  실행되지 않는 취약점도 있었다.
- **수정**: 가드 조건을 "세트가 1개 이상 있을 때만" 스킵하도록 좁히고(`r[name]?.length > 0`),
  세트 초기화 블록과 그립 옵션 설정 블록을 각각 try/catch로 감싸 중간에 에러가 나도 최소
  기본 세트는 반드시 채워지도록 방어했다. (`WorkoutInput.jsx`, `openExercise`)

[2026-08-06 (4)] 운동 종목 DB 정리 12건 + 리포트 레이더차트 보조근 반영 + HOWTO 다건 별칭
- **종목 삭제(4건)**: `팬디컬크로스오버`(가슴), `21컬`(이두), `덤벨플로어프레스`(삼두),
  `밴드푸시다운`(삼두)을 라이브러리/META/설명에서 완전히 제거했다. 사용자 확인: 실사용 이력이
  없어 레거시 매핑 없이 삭제하고, 필요하면 추후 커스텀 종목으로 재추가하는 방식으로 처리.
- **행잉니레이즈 삭제 + 대체**: 이미지 데이터셋에 기본형 GIF가 끝내 없어 종목 자체를 삭제하고
  `행잉레그레이즈`로 대체(`LEGACY_EXERCISE_NAME_MAP`에 매핑 추가). 단, HOWTO 비교군
  "레그레이즈 계열" 표(정적 텍스트 테이블)에는 정보 제공 목적으로 그대로 남겨뒀다(사용자 확인 —
  선택 가능한 종목 목록에서만 빠지고, 비교 설명에서는 제외하지 않음).
- **데드리프트/스모데드리프트 하체 중복 등재**: 루마니안데드리프트와 동일한 기존 패턴대로,
  "등"뿐 아니라 "하체" 카테고리에도 동일 이름으로 등재해 하체 탐색에서도 보이게 했다.
- **웨이티드 표기 통일**: `웨이티드딥스`→`웨이티드(중량) 딥스`, `웨이티드풀업`→`웨이티드(중량)
  풀업`, `하이퍼익스텐션(웨이티드)`→`하이퍼익스텐션(웨이티드-중량)`. 옛 이름은
  `LEGACY_EXERCISE_NAME_MAP`에 추가해 과거 기록과의 호환을 유지했다.
- **플레이트레터럴로우 이미지 수정**: 기존 근사 이미지(덤벨 리어델트 레이즈)가 실제 동작과
  차이가 커서, ExerciseGymGifsDB에서 동작이 더 정확한 `delts/barbell-rear-delt-row`로 교체.
- **힙어브덕션/힙어덕션 병기**: 검색 결과 실제로 서로 다른 근육(외전근 vs 내전근)을 쓰는 별개
  운동으로 확인되어 통합하지 않고, 헬스장 통용 명칭에 맞춰 `힙어브덕션머신(아웃타이)` /
  `힙어덕션머신(이너타이)`로 병기했다.
- **핵스쿼트머신 병기**: 브이스쿼트머신과 실질적으로 동일한 기구로 확인되어
  `핵스쿼트머신(브이스쿼트머신)`으로 병기.
- **아크트레이너 신규 추가**: 일립티컬과 궤적이 달라 별도 기구로 취급, "유산소" 카테고리에
  독립 종목으로 추가(사용자 확인: 병기가 아닌 별도 항목). 이미지 데이터셋에 대응 GIF가 없어
  "이미지 준비중"으로 표시된다.
- **리포트탭 레이더차트 — 보조근(synergist) 개입 반영(신규)**: "가슴/등 운동 시 이두·삼두·
  어깨도 나름 개입되는 점을 감안해서 표시" 요청 반영. `EXERCISE_META.muscleRoles.synergist`를
  8개 원자 부위로 매핑하는 `getSynergistAtoms()`를 신규 추가하고, 주동근 외 부위에 해당 세트
  활동량의 30%를 가산(`SYNERGIST_WEIGHT`)한다. 직접 기록이 없어도(sets=0) 보조근 개입만으로
  레이더에 나타날 수 있도록 `presentBodyParts` 필터를 `sets>0 || secondaryScore>0`으로 확장
  했고, 축 상세 텍스트는 직접 기록이 없는 경우 "보조근 개입만"으로 구분 표시해 실제 수행
  볼륨과 혼동되지 않게 했다. 유산소는 muscleRoles가 없어 대상에서 자연히 제외된다.
  (`exerciseLibrary.js`, `ReportTab.jsx`)
- **HOWTO 검색 — 다건 별칭 지원(신규)**: 기존 `alias`(종목당 1개, 화면 표시용)와 별개로
  검색 전용 `aliases` 배열을 추가해 국내 통용 줄임말(예: 사레레→덤벨사이드레터럴레이즈,
  데드→데드리프트, 힙쓰→바벨힙쓰러스트 등)로도 찾을 수 있게 했다. 1차로 28개 종목에 흔한
  별칭을 등록(사용자 확인 목록 기준). `getExerciseAliases()` 신규 함수 추가, `HowToTab.jsx`
  검색 필터가 `alias` 대신 이 함수를 사용하도록 변경. (`exerciseLibrary.js`, `HowToTab.jsx`)
...
- **루틴 편집 화면(파트 편집 카드) atom별 그룹핑**: 복합 부위 파트(예: 등&이두&코어&유산소)를
  ...
  (원자 부위) 단위로 소제목 + 부위 고유 색상(점 표시)을 넣어 구간을 나눴다. atom 순서는 파트명
  토큰 순서(`getAtomsForPartName`)를 그대로 따른다. (`RoutineSetup.jsx` `PartEditor`)
- **동작 가이드 롱프레스 복원(이미지+설명+팁)**: 2026-08-02에 이미지가 유료 DB 문제로
  전면 삭제됐던 "종목 롱프레스 → 동작 가이드 이미지 토글" 기능을, 이번엔 설명(summary/howTo)
  +팁까지 함께 보여주도록 복원했다. 롱프레스(500ms, 포인터 이벤트로 터치/마우스 공통)로
  열리고, 열린 상태에서 짧게 재탭하면 패널만 닫히며 선택(active)/카드 펼침(expanded) 상태는
  바뀌지 않는다(롱프레스로만 닫히는 방식이 아님). 롱프레스 중 텍스트 선택/iOS 콜아웃 메뉴도
  방지. 공용 로직을 신규 `ExerciseGuideToggle.jsx`(컴포넌트 + `useExerciseGuidePress` 훅)로
  분리해 루틴 편집 화면의 Chip 목록(RoutineSetup.jsx)과 NOTE탭 종목 카드 헤더의 종목명
  (WorkoutInput.jsx) 양쪽에서 공용으로 쓴다.
- **MY탭 커스텀 종목 "정식 종목에 추가됨" 배지**: 커스텀으로 추가했던 종목이 이후 업데이트로
  정식 라이브러리에 동일 이름으로 들어온 경우(예: 플레이트레터럴로우) 배지로 알려준다.
  삭제해도 과거 기록은 이름 문자열로 그대로 저장돼 있어 영향 없음. (`MyPageTab.jsx`)
- (참고) "랫풀다운(내로우언더그립)"처럼 정식 그립 옵션과 1:1로 정확히 일치하지 않는 커스텀
  종목은 이번 배지 로직 대상이 아니다(오탐 방지를 위해 정확히 같은 이름일 때만 표시). 사용자
  본인이 어떤 그립이었는지 확인 후 MY탭에서 직접 삭제 + 이후 "랫풀다운" 정식 종목의 그립칩
  으로 기록하는 방식을 안내함.

[2026-08-06] 버그수정 2건 + 부위별 운동 추이 지표 개선
- **HOWTO 온보딩 배너 미리보기 추가**: HOWTO 탭 상단 배너는 "다시 안 보기"를 누르면
  `users/{uid}.howtoOnboardingDismissed=true`로 저장되어 실제 계정에서는 다시 볼 방법이
  없었다. 배너 문구를 상수(`HOWTO_ONBOARDING_TEXT`)로 분리하고, 관리자 전용 "온보딩 화면
  미리보기"(MY탭 → `Onboarding.jsx` previewMode)에도 같은 문구를 읽기 전용으로 함께
  노출해 언제든 확인할 수 있게 했다. (`HowToTab.jsx`, `Onboarding.jsx`)
- **시작 버튼: 카드는 펼쳐지는데 인풋(스테퍼)이 안 생기는 문제 수정**: 종목 펼침 시 직전
  기록 조회(`getLastRecordForExercise`, Firestore 비동기 호출)가 실패하면 그 이후 세트
  초기화 코드까지 도달하지 못해 인풋이 아예 생성되지 않았다. 조회 실패 시 `last`를
  `null`로 폴백해 세트 초기화가 항상 진행되도록 방어 코드 추가. (`WorkoutInput.jsx`)
- **부위별 운동 추이(리포트 탭 레이더) — 유산소/맨몸 부위 누락 수정**: 기존에는 활동량을
  "무게×횟수(볼륨)" 단일 지표로만 계산해, 유산소·맨몸 위주 부위(코어 플랭크, 풀업 등)는
  볼륨이 항상 0이 되어 표시 자체가 안 됐다. 종목 유형별로 지표를 분리해 누적(근력=볼륨,
  유산소=세트별 소모 칼로리, 맨몸/횟수전용=체중×횟수)하고, 표시 여부 기준도 `볼륨>0`에서
  `세트 수>0`로 변경. 서로 다른 단위를 한 축에 겹치는 문제를 피하기 위해 부위별 값을
  최댓값 대비 0~100 상대 점수로 정규화해서 그리고, 실제 원 수치(볼륨/kcal/체중볼륨)는
  축 라벨 아래 상세 텍스트에 그대로 유지. (`ReportTab.jsx`, `calories.js`에
  `estimateCardioSetKcal` 신규 export)

[2026-08-05 (8)] 종목 이미지 매핑 보강(62건) + 매칭 오류 수정(2건) + 그립 선택 기능 신규
- **이미지 매핑 보강**: 그립 통합/DB 재구축(v2.1)으로 추가된 143개 종목 중 매핑이 비어있던
  62개 종목을 ExerciseGymGifsDB@v1.1.0에서 슬러그를 재확인해 채웠다. 모두 실제 GIF를 열어
  육안으로 동작을 확인했으며, 완전히 동일한 동작 GIF가 없는 경우 같은 부위·유사 동작(장비만
  다른 경우 포함)의 GIF를 근사 매칭으로 대신 연동하고 주석에 "(근사)"로 표시했다. 21컬/
  팬디컬크로스오버/덤벨플로어프레스/밴드푸시다운/행잉니레이즈 5개는 DB에 대응 동작이 없어
  여전히 매핑하지 않았다(기존 정책대로 "이미지 준비중" 유지). (`exerciseImageMap.js`)
- **매칭 오류 수정**: 육안 확인 과정에서 기존 매핑 중 2건이 실제로는 다른 동작 GIF였음을
  발견해 수정했다(이 데이터셋 자체의 폴더-내용 뒤섞임 이슈로 보임, audit.txt의 "mezclados"
  경고와 일치하는 사례).
  - 숄더프레스머신: `lever-shoulder-press` → 실제로는 스쿼트 동작이었음 → `lever-shoulder-press-v-2`(정확한 머신숄더프레스)로 교체
  - 트레드밀: 인클라인워킹과 동일한 경사 트레드밀 GIF를 쓰고 있어 구분이 안 됐음 → 평지
    트레드밀 GIF(`run-equipment`)로 교체, 인클라인워킹은 기존 그대로 유지
  - (조사 중 추가 발견, 신규 매핑 과정에서) 숄더프레스머신(원암)에 쓰려던
    `lever-one-arm-shoulder-press`도 실제로는 다른 동작이라 사용하지 않고, 장비는 다르지만
    동작이 정확한 `dumbbell-one-arm-shoulder-press`(원암덤벨숄더프레스와 동일 GIF)로 대체
- **그립 선택 기능 신규**: 그립 통합 8종목(EXERCISE_META.gripOptions)이 지금까지 HOWTO 탭
  정보 표시에만 쓰이고 실제 기록 시 선택할 수 없었던 문제를 수정. 기록탭 종목 카드 펼침
  영역 최상단에 그립 선택 칩을 추가(세트마다가 아닌 종목당 1회 선택, 직전 기록의 그립을
  기본값으로 이어받음). 선택한 그립은 `workoutLogs.exercises[].grip` 필드로 함께 저장된다
  (그립 옵션이 없는 종목은 필드 자체가 없음, 기존 저장된 기록과 호환). (`WorkoutInput.jsx`,
  `storage.js` 주석 갱신)

[2026-08-05 (7)] 버그수정 — HOWTO 탭 "내 루틴에 추가" 버튼이 하단 네비에 가려 잘리는 문제
- **원인 재분석**: (6)-①에서 고친 "목록→상세 스크롤 미리셋" 문제와는 별개로, `HowToTab.jsx`
  콘텐츠 wrapper(목록/상세 공통)가 하단 패딩을 `32px`로만 잡고 있었음. HOME/MY 등 다른 탭은
  전부 하단 패딩 `100px`로 고정 `BottomNav`(약 60px + 세이프에어리어) 높이를 여유 있게
  확보하는데, HowToTab만 이 컨벤션을 따르지 않아 스크롤을 끝까지 내려도 마지막 버튼이 하단
  네비 뒤에 깔려 보였음.
- **수정**: 목록/상세 화면 wrapper 2곳 모두 하단 패딩을 다른 탭과 동일한 `100px`로 통일.
  (스크롤 리셋 로직은 그대로 유지) (`HowToTab.jsx`)
